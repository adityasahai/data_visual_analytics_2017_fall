...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        for i in range(n):
            # A random int from the entire range will make sure that the samples selected are without replacement.
            row_id = randint(0, n-1)
            samples.append(XX[row_id][:-1])
            labels.append(XX[row_id][-1])
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                # To handle this case, we will create a new DT using the dataset which
                # specifically does not have this record. We will remove this record from
                # a random dataset and replace with a random record from the same dataset,
                # train the DT and classify this record.
                j = randint(0, len(self.bootstraps_datasets)-1)
                random_dataset = self.bootstraps_datasets[j]
                label = self.bootstraps_labels[j]
                index = random_dataset.index(record)
                random_dataset.remove(record)
                label.pop(index)
                k = randint(0, len(random_dataset)-1)
                random_dataset.append(random_dataset[k])
                label.append(label[k])
                dt = DecisionTree()
                dt.learn(random_dataset, label)
                y = np.append(y, dt.classify(record))
            else:
